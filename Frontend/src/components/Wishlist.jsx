...
const Wishlist = () => {
  ...
  //     try {
  //       const response = await axios.get(`/api/wishlist/products/${userId}`);
  //       setWishlist(response.data); // Set fetched wishlist data
  //       console.log(response.data); // Log all products fetched
  //     } catch (error) {
  //       console.error('Error fetching wishlist:', error);
  //     } finally {
  //       setLoading(false);
  ...
  const fetchImages = async (products) => {
    const updatedProducts = await Promise.all(
      products.map(async (product) => {
        try {
          const response = await axios.get(`/api/product/${product.id}/image`, { responseType: 'blob' });
  ...
  useEffect(() => {
    if (wishlist.length > 0) {
      fetchImages(wishlist).then(updatedProducts => {
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <h5
                        ...
                        style={{ fontFamily: "Times New Roman", margin: "0 0 1px 0px", fontSize: "1.2rem" }}
                      >
                        {name}
                      </h5>
                      </div>
